...
from particle_fm.utils.lhco_utils import cluster_data
...
def main(params):
    ...
    data, generation_time = generate_data(
        ...
        batch_size=1024,
        cond=torch.Tensor(normalized_cond),
    ...
    clustered_data = cluster_data(
        data, max_jets=2, max_consts=datamodule.hparams.num_particles, verbose=True
    )

    print(f"clustered_data.shape: {clustered_data.shape}")

    save_path = join(
        data_folder, "lhco", "generated", f"{save_file}-{params.ode_solver}-{params.ode_steps}.h5"
    )
    with h5py.File(save_path, "w") as f:
        f.create_dataset("particle_features", data=clustered_data)
        f.create_dataset("jet_features", data=cond)
    ...
    print(f"Number of generated samples: {len(clustered_data)}")

    print("Calculating Wasserstein metrics")
    # Load idealized data
    sorted_consts = clustered_data[..., [1, 2, 0]]  # eta, phi, pt
