...
from torch.distributions import Distribution, MultivariateNormal, Transform

from .distributions import BoxUniform, DiagNormal, Mixture, NormalizingFlow
...
class NeuralAutoregressiveTransform(MaskedAutoregressiveTransform):
    ...
    def f(self, signal: Tensor, x: Tensor) -> Tensor:
        return self.network(torch.cat(broadcast(x[..., None], signal, ignore=1), dim=-1)).squeeze(
            dim=-1
        )
