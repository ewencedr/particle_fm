...
class MLP(nn.Sequential):
    def __init__(
        ...
        in_features: int,
        out_features: int,
        hidden_features_drop_rate: list,
    ):
        layers = []

        for (in_units, in_drop), (out_units, out_drop) in zip(
            [[in_features, 0]] + hidden_features_drop_rate,
            hidden_features_drop_rate + [[out_features, 0]],
        ):
            layers.extend([nn.Linear(in_units, out_units), nn.Dropout(out_drop), nn.ELU()])
...
class HighLevelClassifier(LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # use a simple feed-forward network as the discriminator
        input_dim = net_config["input_dim"]
        fc_params = net_config["fc_params"]

        self.net = MLP(input_dim, 2, fc_params)
